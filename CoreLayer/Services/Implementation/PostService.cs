    ...
    public class PostService : IPostService
    {
        ...
        public async Task<List<PostDTO>> GetPostsForShow(int? Take = null)
        {
            try
            {
                if (Take != null)
                {
                    List<Post> Post = await _Context.Posts.Where(P => P.Visible).OrderByDescending(P => P.PublishDate).Take((int)Take).ToListAsync();
        ...
        #region (Get Post By Slug)
        public async Task<PostDTO> GetPostBySlugForShow(string Slug)
        {
            try
            {
                Post Post = await _Context.Posts.SingleOrDefaultAsync(P => P.Slug == Slug.ToSlug());
        ...
        public async Task<bool> IncreaseVisit(string Slug)
        {
            try
            {
                Post Post = await GetPostBySlug(Slug);
